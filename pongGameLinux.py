...
# 4 - nunca erra | 5 - supre dificil | 6 - mediano | 7 - Facil | ++ nivel bebe chorão
DIFICULDADE = 7
...
while janela_aberta :
    ...
    if posicao_x_bola + bola_tamanho > POSICAO_X_CPU and (posicao_y_bola + bola_tamanho) > posicao_y_cpu and posicao_y_bola < (posicao_y_cpu + 80 ) and pode_colidir_barra_cpu:
        ...
        # --muda a direção do erro--
        if num_randon_erro_cpu != 0:
            if num_randon_erro_cpu > 0:
                num_randon_erro_cpu = inverte_valor( (random.randrange(0, DIFICULDADE)) * 10 )
            else:
                num_randon_erro_cpu = (random.randrange(0, DIFICULDADE)) * 10
        else:
            num_randon_erro_cpu = (random.randrange(0, DIFICULDADE)) * 10

        print(f"Numero Pixel ERRO_CPU: {num_randon_erro_cpu}")
